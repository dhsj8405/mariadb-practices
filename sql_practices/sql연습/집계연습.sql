...
select avg(salary)
 ...
 -- 그룹바이의 emp_no의 salary를 조회하는것이므로 select emp_no 가능
 select emp_no, avg(salary) as avg_aslary
     from salaries
 group by emp_no;
 
 
-- having
-- 집계결과(결과 임시테이블)에서 row를 선택해야 하는 경우
...
-- 예제
-- salaries 테이블에서 사번이 10060인 직원의 급여 평균과 총합을 출력해라 
-- 문법적오류
-- 의미적으로 맞다(where)
select emp_no ,avg(salary), sum(salary)
  from salaries
 where emp_no= '10060';
-- emp_no 출력하려면 서브쿼리사용해야함 
